...
def test_orderline_orm_can_get_lines(session):
    session.execute(
...
def test_orderline_orm_can_add_lines(session):
    new_line = model.OrderLine('order1', 'TV', 1)
    session.add(new_line)
    session.commit()

    rows = list(session.query(model.OrderLine).all())

    assert new_line == rows[0]
